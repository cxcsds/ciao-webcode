...
<!-- $Id: links.xsl,v 1.68 2004/09/07 21:52:18 dburke Exp $ -->
   ...
   name=<xsl:value-of select="$name"/> context=<xsl:value-of select="$context"/>
	...
	<a>
	  ...
	  <!--*
              ...
              * - and if we are linking to a parameter we use that synopsis instead
	...
	<a>
	  <xsl:attribute name="href">
		...
		<xsl:choose>
		  ...
		  <xsl:otherwise>
		    <xsl:value-of select="concat('/cgi-gen/ciao/download_ciao_',@type,'.cgi')"/>
		  </xsl:otherwise>
		</xsl:choose>
	      </xsl:when>
	      <xsl:otherwise>
